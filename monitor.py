#!/usr/bin/env python3
"""
Monitor DRAGON BALL SUPER CARD GAME — FUSION WORLD (Bandai).

Juego ACTIVO de Bandai Card Games (misma división que One Piece / Naruto CG). Se
vende en las MISMAS tiendas que One Piece. El bot vigila colecciones de Dragon Ball /
feeds de preventa y avisa de NUEVOS listados y RESTOCKS.

Prioridad de PRODUCTO (lo que más interesa al usuario), de mayor a menor:
  1. 🔥 MANGA BOOSTER (sets SB — SB01/SB02/SB03...): cases y booster boxes. MÁXIMA prioridad.
  2. 🚨 CASE / BOOSTER BOX de cualquier set (case, display, caja sellada...).
  3. 🎁 PROMOS / productos promocionales (promotion pack, tournament, championship...).
Se muestran en ese orden y se marcan en el aviso.

Features:
- Filtro por keywords: solo notifica títulos de Dragon Ball / Fusion World
  (required_keywords), descartando merch (figuras, funko, tazas, tomos de manga...
  vía exclude_keywords).
- Prioridad de SITIO (high/medium) solo reparte el chequeo entre workflows (los feeds
  de preventa y las colecciones de manga booster van en HIGH = cada 5 min).
- Detección de RESTOCK (producto agotado que vuelve a stock).
- Baseline SILENCIOSO en la 1ª pasada: como Fusion World ya tiene catálogo, la primera
  vez se absorbe todo lo existente sin avisar; a partir de ahí solo se notifica lo NUEVO
  (preventas de SB03, cases nuevas...) y los restocks.
"""
...
def product_rank(p):
    """Orden de interés: 0 = manga booster (top), 1 = case/box, 2 = promo, 3 = resto."""
...
def check_site(site_cfg, state, config):
    ...
    if products is None:
        # Fallo persistente. Puede ser una colección "adivinada" que aún no existe
        # (404, p.ej. un SB03 futuro) o un bloqueo puntual. Aviso, no error.
    ...
    if required_keywords:
        ...
        log.info(f"  {name}: {len(products)} detectados, {len(filtered)} matchean DRAGON BALL"
    ...
    # Baseline SILENCIOSO en la primera pasada: Fusion World ya tiene catálogo, así que
    # todo lo que existe HOY se absorbe sin avisar y solo se notifica lo NUEVO a partir
    # de ahora (preventas de nuevos sets, cases nuevas...) y los restocks.
    if is_first_run:
        for p in products:
...
def format_notification(site_name, priority, alerts):
    ...
    has_manga = any(a.get("top_priority") for a in alerts)
    has_high_value = any(a.get("high_value") for a in alerts)
    header = "🔄 RESTOCK + " if has_restock else ""
    if has_high_value:
        header = "🚨 CASE/BOX + " + header
    if has_manga:
        header = "🔥 MANGA BOOSTER + " + header
    lines = [f"🐉 {header}<b>DRAGON BALL FW — {site_name}</b> {emoji} [{priority.upper()}]\n"]
    # Manga booster primero, luego cases/boxes, luego promos (lo que más interesa arriba)
    alerts_sorted = sorted(alerts, key=product_rank)
    for p in alerts_sorted[:10]:
        tag = "🔄 VUELVE" if p["alert_type"] == "restock" else "🆕 NUEVO"
        if p.get("top_priority"):
            mark = " 🔥 MANGA BOOSTER"
        elif p.get("high_value"):
            mark = " 🚨 CASE/BOX"
        elif p.get("promo"):
            mark = " 🎁 PROMO"
        else:
        ...
        lines.append(f"• {tag}{mark}{stock_mark} <b>{p['title']}</b>")
